...
\setlength\hsize{0.9\linewidth}
 ...
 \While{at least one service discovered}{
 Add services as the next layer in $L$\;
 Add the outputs of these services to the output set\;
 Discover additional services satisfied by the updated output set\;
 }
  \eIf{Output set satisfies $O$}{
   \Return $L$\;
   }{
   Report no solution\;
 }
...
\setlength\hsize{0.9\linewidth}
 ...
 \For{all sorted layers in L, from $end$ to first}{
 	Identify the inputs from next-to-satisfy set that correspond to services in this layer\;
    \While{not all of these inputs have been satisfied}{
	Find the next service from the previous layers with the highest weight\;
	\If{service outputs satisfy at least one input}{
		Update running QoS values with service QoS (add to running cost, multiply with running availability and reliability)\;
		Add the inputs of service to the set of next inputs to satisfy, each associated with (service time + highest time from satisfied inputs) and current layer position\;
    }
 }
...
\setlength\hsize{0.9\linewidth}
 ...
 \ForAll{sorted layers in $L$, from $end$ to first}{
 	Identify the inputs from next-to-satisfy set that correspond to services in this layer\;
    \While{not all of these inputs have been satisfied}{
    	Find the next service from the previous layer with the highest weight\;
	    \If{the outputs of this service satisfy at least one of the inputs for this layer}{
			Add service node to graph\;
			Add edges connecting this service node to the nodes whose inputs it satisfies\;
			Add the inputs of the service node to the set of next inputs to satisfy, each associated with the service node and current layer position\;
 }}}
